...
class SignInfo:
    ...
    def __init__(self, option: int, num_hands: int, handface=None, raw_vid_info: RawVidInfo = None):
        """
        :param option: 0 for initializing from matlab file handface
                       1 for initializing from RawVidInfo class
        ...
        if option == 0:
            # raise error if handface is None when initializing from matlab
            if handface is None:
                raise "parameter handface can not be None when initializing from matlab file"

            # otherwise init
            self.init_from_matlab(handface)
        elif option == 1:
            # raise error if raw_vid_info is None when initializing from vid
            if raw_vid_info is None:
                raise "parameter raw_camera_info can not be None when initializing from camera"

            # otherwise init
            self.init_from_vid(raw_vid_info)
        else:
    ...
    def init_from_vid(self, raw_vid_info: RawVidInfo, dom_hand: str = "r"):
        """
        :param raw_vid_info: RawVidInfo class containing info for this sign
        :param dom_hand: which hand is dominant, either "r" or "l"
        """

        if dom_hand == "r":
            self.dom_hand_pos = raw_vid_info.right_hand
            self.ndom_hand_pos = raw_vid_info.left_hand
        elif dom_hand == "l":
            self.dom_hand_pos = raw_vid_info.left_hand
            self.ndom_hand_pos = raw_vid_info.right_hand

        self.dom_hand_pos = normalize_length(self.dom_hand_pos, 20)
        self.dom_hand_pos = ((self.dom_hand_pos - raw_vid_info.face) / raw_vid_info.scaling_dist) * 2.0
        ...
        if self.num_hands == 2:
            self.ndom_hand_pos = normalize_length(self.ndom_hand_pos, 20)
            self.ndom_hand_pos = ((self.ndom_hand_pos - raw_vid_info.face) / raw_vid_info.scaling_dist) * 1.4
