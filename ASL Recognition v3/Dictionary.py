...
class Dictionary:
    def __init__(self, *sessions):
        ...
        for i in range(1113):
            ...
            for session in sessions:
                temp = SignInfo(0, num_hands, session.handface[i])
    ...
    def sign_to_word(self, top_n: int, num_hands: int, unknown: SignInfo, is_subsequence=False) -> tuple[list[str], list[str]]:
        """
        ...
        :param is_subsequence: whether the query sign should be treated as a subsequence or not; defaulted to False
                               when it is true, subsequence_search() is called instead of calc_distance
        ...
        for sign in self.vocab:
            ...
            if sign.num_hands == num_hands:
                ...
                if is_subsequence:
                    score = sign.subsequence_search(unknown)
                else:
